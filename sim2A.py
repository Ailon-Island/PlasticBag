...
import taichi as ti
import numpy as np
from utils import mat3, scalars, vecs, mats, T, TetMesh, get_mouse_ray, get_reduce_min_inplace
import utils
...
class MpmLagSim:
    ...
    def set_air(self, n_air_particles, p_rho_air, p_vol_air, E_air, nu_air):
        self.n_air_particles = n_air_particles
        self.p_rho_air = p_rho_air
        self.p_vol_air = p_vol_air
        self.mu_air, self.la_air = E_air / (2 * (1 + nu_air)), E_air * \
            nu_air / ((1 + nu_air) * (1 - 2 * nu_air))
    ...
    def update_scene(self, show_air=False):
